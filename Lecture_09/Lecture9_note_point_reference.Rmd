---
title: "Lecture9_note_point_reference"
author: "Shukyee Chan"
date: "2023-11-09"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(fig.width = 9, fig.height = 3, message = FALSE, warning = FALSE)
```

## Load the Packages

```{r}
library(tidyverse)
library(ggthemes)
theme_set(theme_map())
```

## Download UCDP Data

```{r, eval=TRUE}
# Location: https://ucdp.uu.se/downloads/

download.file(
  "https://ucdp.uu.se/downloads/ged/ged221-csv.zip",
  destfile = "Lec_09/data/ged221-csv.zip")

# Decompress the downloaded ZIP file
unzip("Lec_09/data/ged221-csv.zip", exdir = "Lec_09/data")

# Codebook available on the website

```

## Load the Conflict Data

```{r}
d = read_csv("Lecture_09/data/GEDEvent_v22_1.csv")
names(d)
```

## Plot Events on a Map

```{r}
# Create a dataframe named d_event_2021 that contains all events in the year of 2021
d_event_2021 = d |> filter(year == 2021)
```

## Load the World Map

```{r}
world = map_data("world")

names(world)
# Summary statistics
summary(world)

# Problem: Max. longitude > 180! Remove those
world = world |> filter(long <= 180)
```

```{r}
world_china = world |> 
  filter(region == "China")

world_china |>
  ggplot() +
  geom_point(aes(x = long, y = lat))
```

```{r, fig.width=15, fig.height=12}
# Plot the world map
world |>
  ggplot(aes(x = long, y = lat)) +
  geom_map(
    map = world,
    aes(map_id = region),
    color = "white", fill = "gray", linewidth = 0.1
  )
```

```{r, fig.width=15, fig.height=12}
#  Add map projection
world |>
  ggplot(aes(x = long, y = lat)) +
  geom_map(
    map = world,
    aes(map_id = region),
    color = "white", fill = "gray", linewidth = 0.1
  ) +
  coord_map()
```

```{r, fig.width=15, fig.height=12}
# There are a variety of projections available (for pragmatic and aethetic purposes)
world |>
  ggplot(aes(x = long, y = lat)) +
  geom_map(
    map = world,
    aes(map_id = region),
    color = "white", fill = "gray", linewidth = 0.1
  ) +
  coord_map("moll")

world |>
  ggplot(aes(x = long, y = lat)) +
  geom_map(
    map = world,
    aes(map_id = region),
    color = "white", fill = "gray", linewidth = 0.1
  ) +
  coord_map("gilbert")

world |>
  ggplot(aes(x = long, y = lat)) +
  geom_map(
    map = world,
    aes(map_id = region),
    color = "white", fill = "gray", linewidth = 0.1
  ) +
  coord_map("orth")
```

## Plot Conflict Events as Points on the map

```{r}
# plot the events as points on the map
ggplot() +
  geom_point(data = d_event_2021, aes(x = longitude, y = latitude))
```

### Stack the points onto the map

```{r, fig.width=15, fig.height=12}
world |>
  ggplot(aes(x = long, y = lat)) +
  geom_map(
    map = world,
    aes(map_id = region),
    color = "white", fill = "gray", linewidth = 0.1
  ) +
  # Below: Stack a layer onto the map
  geom_point(data = d_event_2021, aes(x = longitude, y = latitude)) +
  coord_map()
```

### Make the Plot Prettier

```{r, fig.width=15, fig.height=12}
world |>
  ggplot(aes(x = long, y = lat)) +
  geom_map(
    map = world,
    aes(map_id = region),
    color = "white", fill = "gray", linewidth = 0.1
  ) +
  geom_point(
    data = d_event_2021, 
    aes(x = longitude, y = latitude),
    alpha = 0.3
    ) +
  coord_map()
```

## Plot Conflict Mortality

```{r}
# Use number of deaths as size of points.
# calculate total number of deaths
d_event_2021 = d_event_2021 |>
  mutate(Deaths = deaths_a + deaths_b + 
           deaths_civilians + deaths_unknown)

summary(d_event_2021$Deaths)

d_event_2021 |>
  ggplot() +
  geom_density(aes(x = Deaths)) +
  theme_bw()
```

```{r, fig.width=15, fig.height=12}
world |>
  ggplot(aes(x = long, y = lat)) +
  geom_map(
    map = world,
    aes(map_id = region),
    color = "white", fill = "gray", size = 0.1
  ) +
  geom_point(
    data = d_event_2021, 
    aes(x = longitude, y = latitude, size = Deaths),
    alpha = 0.3
    ) +
  coord_map() +
  theme(legend.position = "bottom")
```

```{r, fig.width=12, fig.height=8}
# Make the figure more informative: taking logarithm of the Death variable
world |>
  ggplot(aes(x = long, y = lat)) +
  geom_map(
    map = world,
    aes(map_id = region),
    color = "white", fill = "gray", linewidth = 0.1
  ) +
  geom_point(
    data = d_event_2021, 
    aes(x = longitude, y = latitude, size = log(Deaths+1)),
    alpha = 0.1
    ) +
  coord_map() +
  theme(legend.position = "bottom")
```

```{r, fig.width=12, fig.height=8}
# use a more informative color scheme
world |>
  ggplot(aes(x = long, y = lat)) +
  geom_map(
    map = world,
    aes(map_id = region),
    color = "white", fill = "gray", linewidth = 0.1
  ) +
  geom_point(
    data = d_event_2021, 
    aes(x = longitude, y = latitude, 
        size = log(Deaths+1), color = log(Deaths+1)),
    alpha = 0.1
    ) +
  scale_color_viridis_c(direction = -1, option = "A") +
  coord_map() +
  theme(legend.position = "bottom")
```
