...
d <- read_csv("data/classification.txt")
...
```{r, echo=TRUE}
d_tally <- d_tally |>
  mutate(label = as.numeric(str_extract(label, "\\d")))


d_tally <- d_tally |>
  mutate(probability = as.numeric(str_extract(probability, "\\d+\\.\\d+")))
...
d_tally <-d_tally |>
  separate(name_image, into = c("state", "district","name_image"),
